...
– [ ] When the slider control of inputColor is moved, change the background color of the box. Hint: You need to work 
    with the hsl() functional notation for colors.
– [ ] When the slider control of inputRadius is moved, change the border radius of the box. If the control 
    reaches the right end, make the box look like a circle.
– [ ] When the slider control of inputRotation is moved, rotate the box. Hint: You may need to work with 
    the CSS property "transform" and a value that contains "rotate" and "deg".
...
inputColor.addEventListener("input", () => {
  const colorChange = inputColor.value;
  box.style.backgroundColor = `hsl(${colorChange} 80% 40%)`;
});
inputRadius.addEventListener("input", () => {
  const radiusChange = inputRadius.value;
  box.style.borderRadius = `${radiusChange}%`;
});
